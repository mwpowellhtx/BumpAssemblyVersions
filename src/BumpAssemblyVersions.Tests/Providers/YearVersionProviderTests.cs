...
namespace Bav
{
    ...
    /// </summary>
    /// <inheritdoc />
    public class YearVersionProviderTests : TimestampVersionProviderTestsBase<YearVersionProvider>
    {
...
#pragma warning disable xUnit1008 // Test data attribute should only be used on a Theory
        ...
        public override void Verify_Version_element_After_Trying_to_Change(string beforeChange
            , string afterChange, InitializeProviderCallback<YearVersionProvider> init, bool expectedTried)
        {
        ...
        public static IEnumerable<object[]> BeforeAndAfterProviderChanged
        {
            get
            {
                IEnumerable<object[]> GetAll()
                {
                    ...
                    yield return GetOne($"{now.Year - 1}", $"{now.Year}").ToArray();
                    yield return GetOne($"{now.Year}", $"{now.Year}").ToArray();
                }
