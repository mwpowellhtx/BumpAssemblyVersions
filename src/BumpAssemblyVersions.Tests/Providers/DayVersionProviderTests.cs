...
namespace Bav
{
    ...
    /// </summary>
    /// <inheritdoc />
    public class DayVersionProviderTests : TimestampVersionProviderTestsBase<DayVersionProvider>
    {
        ...
        public static IEnumerable<object[]> BeforeAndAfterProviderChanged
        {
            get
            {
                IEnumerable<object[]> GetAll()
                {
                    ...
                    yield return GetOne($"{now.Day - 1}", $"{now.Day}").ToArray();
                    yield return GetOne($"{now.Day}", $"{now.Day}").ToArray();
                }
