...
namespace Bav
{
    ...
    /// </summary>
    /// <inheritdoc />
    public class MinutesSinceMidnightVersionProviderTests : TimestampVersionProviderTestsBase<
        MinutesSinceMidnightVersionProvider>
    {
...
#pragma warning disable xUnit1008 // Test data attribute should only be used on a Theory
        ...
        public override void Verify_Version_element_After_Trying_to_Change(string beforeChange, string afterChange,
            InitializeProviderCallback<MinutesSinceMidnightVersionProvider> init, bool expectedTried)
        {
        ...
        public static IEnumerable<object[]> BeforeAndAfterProviderChanged
        {
            get
            {
                IEnumerable<object[]> GetAll()
                {
                    ...
                    int GetNextAdjustment(double totalMinutes)
                    {
                        const int sixty = 60;
                        const int thirty = 30;
                        ...
                        if (totalMinutes > sixty)
                        {
                            return -thirty;
                        }

                        if (totalMinutes > fifteen)
                        {
                            return -five;
                        }
                    ...
                    IEnumerable<double> CalculateDeltaMinutesRange(DateTime a, DateTime b)
                    {
                        var delta = Abs((a - b).TotalMinutes);
                        do
                        {
                    ...
                    foreach (var s in CalculateDeltaMinutesRange(now, midnight).Distinct())
                    {
                        // Now and Then...
                        var nowAndThen = midnight.AddMinutes(s);
